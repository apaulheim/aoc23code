...
for (let i = 0; i < lines2.length; i++) {
  ...
  let firstNum = wordToNum.has(result[0][1])
    ? wordToNum.get(result[0][1])
    : parseInt(result[0][1]);
  let lastNum = wordToNum.has(result[result.length - 1][1])
    ? wordToNum.get(result[result.length - 1][1])
    : parseInt(result[result.length - 1][1]);
